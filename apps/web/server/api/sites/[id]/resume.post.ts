...
import { resumeSite, SiteNotFoundError } from '~/server/data/sites'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const site = await resumeSite(id)
    await publishCommand({ type: 'RESUME_SITE', payload: buildIdPayload(site.id) }, resolveMqttConfig(event))
    return { site }
  } catch (error) {
